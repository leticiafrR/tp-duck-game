...
Socket::Socket(const char* hostname, const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         * */
        local_skt = socket(addr->ai_family, addr->ai_socktype, addr->ai_protocol);
        if (local_skt == -1) {
            continue;
        }
         ...
         * */
        s = connect(local_skt, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
...
Socket::Socket(const char* servname) {
    ...
    while (resolver.has_next()) {
         ...
         **/
        int optval = 1;
        s = setsockopt(local_skt, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
        if (s == -1) {
            continue;
        }
         ...
         * */
        s = bind(local_skt, addr->ai_addr, addr->ai_addrlen);
        if (s == -1) {
            continue;
        }
...
int Socket::recvall(void* data, unsigned int sz, bool* was_closed) {
    ...
    *was_closed = false;
    ...
    while (received < sz) {
        int s = recvsome(reinterpret_cast<char*>(data) + received, sz - received, was_closed);
